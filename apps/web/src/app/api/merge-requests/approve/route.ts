...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Cancel path: ConfirmDialog never submits without confirmed=1.
    if (!confirmed) {
      return formRedirect(request, redirectTo, { flash: "not_confirmed" });
    }

    const result = await services.mrMutations.approve(user.id, {
      gitlabInstanceId,
      gitlabProjectId,
    ...
    if (result.ok) {
      return formRedirect(request, redirectTo, { flash: "approve_ok" });
    }
